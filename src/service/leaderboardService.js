...
export const getOrgRankingFilterService = async (provinceId, categoryId) => {
  try {
    const params = new URLSearchParams();
    if (provinceId) {
      params.append("provinceId", provinceId);
    ...
    const data = await apiRequest(`/leaderboard/organizer-rankings/filter?${params.toString()}`, "GET");
    return data;
  } catch (e) {
    console.log(e);
  }
